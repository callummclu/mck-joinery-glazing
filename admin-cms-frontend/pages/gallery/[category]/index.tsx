...
export const GalleryCategories: NextPage = () => {
    ...
    const {data,error} = useSWR(`https://mck-joinery-glazing-backend.herokuapp.com/gallery/all/${category}`,fetcher)
...
<Center my="xl" style={{display:"flex",flexWrap:"wrap"}}>
    {data?.slice((currPage - 1) * 10, currPage*10 || data.length)?.map((e:any) => <Image radius="md" caption={e.title} key={e.id} onMouseEnter={()=>console.log(e)} onClick={()=>window.location.href = (`${window.location.origin}/gallery/${e.categoryType}/${e.id}`)} p="xs" width={300} height={180} src={e.image}/>)}
